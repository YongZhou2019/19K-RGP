...
chromosomes_present <- sort(unique(X$bim[,1]))

NEFF = data.frame(
  chr = chromosomes_present,
  Meff  = numeric(length(chromosomes_present)),
  Neff_LJ = numeric(length(chromosomes_present)),
  Meff_Gao = integer(length(chromosomes_present))
)
win_df_list = list()

for(i in seq_along(chromosomes_present)) {
  focus_chr <- chromosomes_present[i]
  cat("###  Chromosome" , focus_chr, " ###\n")

  select_chr = X$bim[,1] == focus_chr

if(sum(select_chr) < 2){
    cat("Skipping Chromosome", focus_chr, "due to insufficient SNPs\n")
    next
  }
...
Xchr = list(bed = X$bed[, select_chr],
  ...
  NEFF$Meff[[i]] = sum(win_df$Meff_Cheverud, na.rm = TRUE)
  NEFF$Neff_LJ[[i]] = sum(win_df$Meff_LJ, na.rm = TRUE)
  NEFF$Meff_Gao[[i]] = sum(win_df$Meff_Gao, na.rm = TRUE)

  win_df_list[[ as.character(focus_chr) ]] = win_df
}
...
cat("A possible suggestive threshold (e.g. alpha=0.01): \n")
print( -log10(0.01/ans$tot_Neff_LJ) )
